...
public:
    ...
    template <typename Array>
    gubg::Tribool array(Array &array, bool oc, const std::string &name, const std::string &type)
    {
        MSS_BEGIN(gubg::Tribool, "");L(C(name)C(type)C(oc));

        if (stop)
            return gubg::Tribool::maybe();
